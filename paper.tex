...
\begin{proof}
  ...
  中位回数の定義より，このうち中位回数は$kr^m(\succsim)$である．したがってこれから
  $\mathrm{pop}(f(\succsim)) = kr^m(\succsim)$を示していくことになる．まず中位の定義から
  $kl^m(\succsim),\ kr^m(\succsim) \in\{k(\succsim_1),\ldots,k(\succsim_n)\}$かつ
  \begin{gather}
